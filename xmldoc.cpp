...
static HRESULT AddPinInfo(IXMLDOMDocument* pDoc,IXMLDOMElement* pParent,const PININFO& p)
{
	...
	if(p.globalcinstances_valid)
	{
		...
		hr = AddAttribute(pDoc,L"PossibleCount",_variant_t(p.cinstances.PossibleCount),globalcinstances);
		hr = AddAttribute(pDoc,L"CurrentCount",_variant_t(p.cinstances.CurrentCount),globalcinstances);
